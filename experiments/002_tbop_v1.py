...
# TBop specific
TBOP_GAMMA = 0.01          # EMA adaptivity rate (weights new gradient)
TBOP_TAU_ACT = 0.5         # activation threshold (0 -> ±1)
TBOP_TAU_DEACT = 1.0       # deactivation threshold (±1 -> 0), higher = more inertia
TBOP_TAU_FINAL = 0.1       # final threshold after cosine decay
...
class BitLinearTBop(nn.Module):
    ...
    def __init__(self, in_features, out_features, bias=False):
        ...
        w_scaled = (w_cont / scale).clamp(-1, 1)
        ternary_init = torch.sign(w_scaled) * torch.round(w_scaled.abs())

        # Apply target sparsity: randomly zero out some weights
        # The Kaiming init already produces ~33% zeros; adjust if needed
    ...
    def forward(self, x):
        # Use ternary weights with STE for gradient computation
        w = self.ternary_weight.float()
        w_grad = w.requires_grad_(True)
        self._w_for_grad = w_grad
        return F.linear(x, w_grad, self.bias)
...
def main():
    ...
    ste_log, _ = train_ste_baseline(train_data, val_data, vocab_size, itos)
